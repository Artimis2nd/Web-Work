  ...
  async function load() {
    ...
    try {
      ...
      const columnWidths = {
        ...
        'วันที่': '90px',
        'โครงการ': '180px',
        'รายละเอียด': '300px', // เพิ่มความกว้าง
        'ค่าแรงดิบ': '100px',
        'ปกติ+OT': '100px',
        'เหมา': '100px',
        'รวมทั้งหมด': '110px',
      ...
      const defaultWorkerWidth = '120px'; // ความกว้างสำหรับคอลัมน์คนงาน
      ...
      const tableRows = logs.map((log, index) => {
        ...
        log.Workers.forEach(w => {
          ...
          let displayValue = '';
          if (w.WageType === 'fixed') {
            displayValue = `${Utils.money(w.FixedAmount)}-เหมา`;
            totalFixed += w.RawWage;
          } else {
            displayValue = Utils.money(w.RawWage);
