(() => {
  ...
  let compressedImages = []; // array of base64 strings
  ...
  function renderImagePreviews() {
    ...
    grid.innerHTML = compressedImages.map((base64, idx) => `
      <div class="image-preview">
        <img src="${base64}" alt="รูปที่ ${idx + 1}">
  ...
  async function loadEditGroup(groupId) {
    try {
      ...
      if (data.imageUrls && Array.isArray(data.imageUrls)) {
        compressedImages = data.imageUrls.filter(u => u);
        renderImagePreviews();
      }
