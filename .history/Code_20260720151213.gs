/**
 * ระบบคิดค่าแรงคนงาน — Backend (Google Apps Script) v2.1
 * =======================================================
 * 
 * วิธีติดตั้ง:
 ...
 * - รูปจะถูกจัดเก็บในโฟลเดอร์: {วันที่}_A{ลำดับ}_{GroupID4ตัวแรก}/
 * - ชื่อไฟล์: {seq}_{groupId}_{index}.jpg
...
function setupSheets() {
  ...
  // --- DailyLogs (v2) ไม่มีคอลัมน์ ImageUrls แล้ว ---
...
function addLogs(payload) {
  ...
  // อัปโหลดรูปภาพ → เก็บ URLs ในชีต Images
  var imageUrls = [];
  if (images.length > 0) {
...
function getLogGroup(payload) {
  ...
  // อ่าน ImageUrls จากชีต Images (ไม่ใช่ DailyLogs)
...
function updateLogGroup(payload) {
  ...
  // จัดการรูปภาพ — แยก Base64 (ต้องอัปโหลดใหม่) กับ URL (ของเก่า)
  var base64Images = [];
  var existingUrls = [];
...
// โฟลเดอร์: {วันที่}_A{ลำดับ}_{GroupID4ตัวแรก}/
// ชื่อไฟล์: {seq}_{groupId}_{index}.jpg
// ============================================================

/**
 * หาลำดับถัดไปของวันนั้น โดยเช็คชื่อโฟลเดอร์ที่มี {date} ขึ้นต้น
 */
function getNextSequence(dateStr) {
  var parent = DriveApp.getFolderById(DRIVE_FOLDER_ID);
  var prefix = (dateStr || Utilities.formatDate(new Date(), 'GMT+7', 'yyyy-MM-dd')) + '_A';
  var maxSeq = 0;
...
/**
 * สร้างโฟลเดอร์สำหรับ group นี้
 * ชื่อ: {date}_A{seq}_{groupId4}/
 */
function getOrCreateGroupFolder(dateStr, groupId) {
  var parent = DriveApp.getFolderById(DRIVE_FOLDER_ID);
  var shortId = groupId.substring(0, 4);
  var seq = getNextSequence(dateStr);
  var folderName = dateStr + '_A' + seq + '_' + shortId;

  // ตรวจสอบว่ามีโฟลเดอร์นี้แล้วหรือไม่ (สำหรับ updateLogGroup)
...
/**
 * หาโฟลเดอร์ที่มี groupId อยู่ในชื่อ (ใช้สำหรับ updateLogGroup)
 */
function findGroupFolder(dateStr, groupId) {
  var parent = DriveApp.getFolderById(DRIVE_FOLDER_ID);
  var prefix = (dateStr || '') + '_A';
  var shortId = groupId.substring(0, 4);
  var folders = parent.getFolders();
  while (folders.hasNext()) {
    var f = folders.next();
    var name = f.getName();
    if (name.indexOf(shortId) !== -1 && name.indexOf(prefix) === 0) {
      return f;
    }
  }
  // ถ้าไม่พบ → สร้างใหม่
  return getOrCreateGroupFolder(dateStr, groupId);
}
...
function uploadImages(base64Array, groupId, date) {
  ...
  var folder = findGroupFolder(date, groupId);
  var urls = [];

  base64Array.forEach(function(base64, idx) {
    try {
      var url = uploadSingleImage(base64, groupId, idx, folder);
      urls.push(url);
    } catch (e) {
  ...
  if (base64Array.length > 0 && urls.length === 0) {
    throw new Error('อัปโหลดรูปภาพล้มเหลวทั้งหมด');
  }
...
function uploadSingleImage(base64, groupId, index, folder) {
  ...
  // ดึง seq จากชื่อโฟลเดอร์
  var folderName = folder.getName();
  var seq = '1';
  var seqMatch = folderName.match(/_A(\d+)_/);
  if (seqMatch) seq = seqMatch[1];

  var fileName = seq + '_' + groupId + '_' + index + '.' + ext;
