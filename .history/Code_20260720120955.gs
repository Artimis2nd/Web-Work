...
// CONFIGURATION
// ============================================================
var DRIVE_FOLDER_ID = '1Os8Ntkx2DR5xbzK0JHE_hPG1OjA0WAgp'; // โฟลเดอร์สำหรับเก็บรูปภาพ
...
function calcTotal(rawWage, workerName) {
  if (workerName && workerName.indexOf(FERN_NAME) !== -1) {
    return rawWage; // ยกเว้น markup
  }
...
function addLogs(payload) {
  ...
  // อัปโหลดรูปภาพไปยัง Drive
  var imageUrls = [];
  if (images.length > 0) {
    imageUrls = uploadImages(images, groupId);
  }
...
function updateLogGroup(payload) {
  ...
  // อัปโหลดรูปใหม่ (รวมของเก่าที่ส่งกลับมาเป็น Base64)
  var imageUrls = [];
  if (images.length > 0) {
    // แยกว่าเป็น Base64 หรือ URL แล้ว
    images.forEach(function(img) {
      if (img.indexOf('data:') === 0) {
        // เป็น Base64 ต้องอัปโหลด
        imageUrls.push(uploadSingleImage(img, groupId));
      } else {
...
// ============================================================
function uploadImages(base64Array, groupId) {
  var urls = [];
  base64Array.forEach(function(base64, idx) {
    try {
      var url = uploadSingleImage(base64, groupId + '_' + idx);
      urls.push(url);
    } catch (e) {
...
function uploadSingleImage(base64, fileName) {
  // แยก base64 header
  ...
  var blob = Utilities.newBlob(data, 'image/' + ext, 'wage_' + fileName + '.' + ext);
